...
Research from Zou's group at Stanford supports this. They found that AI is particularly effective at catching objective, verifiable problems in drafts: the kinds of gaps and inconsistencies that are easy to miss when you have been living inside a document for months {cite}`Myers_2026_HAI_peer_review`.
...
NIH makes this explicit: AI is prohibited in peer review under NOT-OD-23-149. NSF takes the same position. Most major journals have adopted similar policies, and the ones that have not yet done so are likely to follow.

```{admonition} If You're at U-M
:class: note

The U-M OVPR specifically discourages AI use in internal peer review for the same confidentiality and bias reasons. See [AI Resources at the University of Michigan](../part4/ch27_um_resources.md) for current UM policy guidance.
```
...
Research from Zou's group at Stanford maps this limitation clearly. They found that AI does its best work on the objective, checkable parts of review, catching a number in a table that does not match the text, or flagging an equation that contradicts an earlier one. But when it comes to assessing whether a finding is genuinely novel or whether the work actually moves a field forward, AI is much less reliable {cite}`Myers_2026_HAI_peer_review`. There is also a subtler risk worth flagging: AI assessments of research significance can veer toward sycophancy, producing feedback that sounds authoritative but is really pattern-matching against what polished academic writing looks like rather than real engagement with the ideas.
...
There is also growing experimental evidence that AI assistance can improve review quality when used thoughtfully. At ICLR 2025, Zou and collaborators provided AI-assisted feedback to human reviewers across roughly 20,000 submissions and found measurable improvements in both the quality of reviews and the level of reviewer engagement {cite}`Myers_2026_HAI_peer_review`. The important caveat is that the gains were concentrated in the more objective parts of review: catching surface inconsistencies, technical errors, and structural gaps rather than the harder questions about whether a paper makes a meaningful contribution to the field.
